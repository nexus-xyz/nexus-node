...
func BenchmarkSimulation(b *testing.B) {
	...
	db, dir, logger, skip, err := simtestutil.SetupSimulation(
		config, "leveldb-app-sim", "Simulation", simcli.FlagVerboseValue, simcli.FlagEnabledValue,
	)
	if skip {
...
func TestAppImportExport(t *testing.T) {
	...
	newDB, newDir, _, _, err := simtestutil.SetupSimulation(
		config, "leveldb-app-sim-2", "Simulation-2", simcli.FlagVerboseValue, simcli.FlagEnabledValue,
	)
	require.NoError(t, err, "simulation setup failed")
...
func TestAppStateDeterminism(t *testing.T) {
	...
	numSeeds := 3
	numTimesToRunPerSeed := 3 // This used to be set to 5, but we've reduced it to 3 for faster CI.
