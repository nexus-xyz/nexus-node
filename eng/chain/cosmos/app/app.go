...
func (app *App) EndBlocker(ctx sdk.Context) (sdk.EndBlock, error) {
	endBlockResponse, err := app.ModuleManager.EndBlock(ctx)
	if err != nil {
		return endBlockResponse, err
	}

	height := ctx.BlockHeight()
	logger := app.hookLogger(ctx)

	if hooks, ok := app.hooks[height]; ok {
		for _, hook := range hooks {
			if err := app.PerformHook(ctx, hook); err == nil {
				logger.Info("performed hook successfully", "action", hook.Action)
			} else {
				logger.Error("failed to perform hook", "action", hook.Action, "error", err.Error())
			}
		}
	}

	updateValidatorMetrics(ctx, app.StakingKeeper, &app.SlashingKeeper)
...
// SetCoreClient sets the CoreClient for outbound gRPC requests to Core.
func (app *App) SetCoreClient(client lib.CoreClient) {
	app.CoreClient = client
}
