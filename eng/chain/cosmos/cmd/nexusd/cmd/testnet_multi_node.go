...
func NewTestnetMultiNodeCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use: "multi-node",
		Short: "Initialize config directories & files for a multi-validator testnet running locally " +
			"via separate processes (e.g. Docker Compose or similar)",
...
Example:
	nexusd multi-node --v 4 --output-dir ./.testnets ` +
			`--validators-stake-amount 1000000,200000,300000,400000 --list-ports 47222,50434,52851,44210
	`,
	...
	cmd.Flags().String(flagNodeDirPrefix, "validator",
		"Prefix the directory name for each node with (node results in node0, node1, ...)")
	cmd.Flags().String(flagValidatorsStakeAmount, "100000000,100000000,100000000,100000000",
		"Amount of stake for each validator")
	cmd.Flags().String(flagStartingIPAddress, "localhost",
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.0001%s", sdk.DefaultBondDenom),
		"Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum "+
			"(e.g. 0.01photino,0.001stake)")
...
func initTestnetFiles(
	...
	if err := initGenFiles(
		clientCtx, mbm, args.chainID, genAccounts, genBalances, genFiles, args.numValidators,
	); err != nil {
		return err
	}
	// copy gentx file
...
func collectGenFiles(
	...
	for i := 0; i < numValidators; i++ {
		...
		nodeAppState, err := genutil.GenAppStateFromConfig(
			clientCtx.Codec, clientCtx.TxConfig, nodeConfig, initCfg, appGenesis, genBalIterator,
