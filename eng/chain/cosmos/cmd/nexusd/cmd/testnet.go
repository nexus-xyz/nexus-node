...
func NewInPlaceTestnetCmd() *cobra.Command {
	...
	cmd.Long = `The test command modifies both application and consensus stores within a local mainnet ` +
		`node and starts the node, with the aim of facilitating testing procedures. This command ` +
		`replaces existing validator data with updated information, thereby removing the old ` +
		`validator set and introducing a new set suitable for local testing purposes. By altering ` +
		`the state extracted from the mainnet node, it enables developers to configure their ` +
		`local environments to reflect mainnet conditions more accurately.`

	cmd.Example = fmt.Sprintf(
		`%sd in-place-testnet testing-1 cosmosvaloper1w7f3xx7e75p4l7qdym5msqem9rd4dyc4mq79dm `+
			`--home $HOME/.%sd/validator1 `+
			`--validator-privkey=6dq+/KHNvyiw2TToCgOpUpQKIzrLs69Rb8Az39xvmxPHNoPxY1Cil8FY+4DhT9YwD6s0tFABMlLcpaylzKKBOg== `+
			`--accounts-to-fund="cosmos1f7twgcq4ypzg7y24wuywy06xmdet8pc4473tnq,cosmos1qvuhm5m644660nd8377d6l7yz9e9hhm9evmx3x"`,
		"nexus", "nexus",
	)

	cmd.Flags().String(flagAccountsToFund, "",
		"Comma-separated list of account addresses that will be funded for testing purposes")
	return cmd
}
...
func initAppForTestnet(app *app.App, args valArgs) *app.App {
	...
	iterator = stakingStore.Iterator(
		stakingtypes.ValidatorQueueKey,
		storetypes.PrefixEndBytes(stakingtypes.ValidatorQueueKey),
	)
	for ; iterator.Valid(); iterator.Next() {
		stakingStore.Delete(iterator.Key())
	}
	...
	handleErr(app.DistrKeeper.SetValidatorHistoricalRewards(
		ctx, validator, 0, distrtypes.NewValidatorHistoricalRewards(sdk.DecCoins{}, 1),
	))
	handleErr(app.DistrKeeper.SetValidatorCurrentRewards(
		ctx, validator, distrtypes.NewValidatorCurrentRewards(sdk.DecCoins{}, 1),
	))
	handleErr(app.DistrKeeper.SetValidatorAccumulatedCommission(
		ctx, validator, distrtypes.InitialValidatorAccumulatedCommission(),
	))
	handleErr(app.DistrKeeper.SetValidatorOutstandingRewards(
		ctx, validator, distrtypes.ValidatorOutstandingRewards{Rewards: sdk.DecCoins{}},
	))
